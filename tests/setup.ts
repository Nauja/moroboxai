...
global.DATA_GAMES_DIR = nodePath.join(DATA_DIR, "games");
...
beforeEach(() => {
    return tempDir({ preserve: true }, async (path) => {
        ...
        const AGENTS_DIR = nodePath.join(MOROBOXAI_DIR, "agents");
        ...
        (platform.AGENTS_DIR as any) = AGENTS_DIR;
        ...
        fs.writeFileSync(
            ...
            ["games", "boots", "agents"]
                .map(
                    (dir) =>
                        `${new URL(dir, global.FILE_SERVER_URL).toString()}/`
                )
                .join("\n")
        );
    });
